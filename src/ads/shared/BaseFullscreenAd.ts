import { adEventDispatcher } from '../../internal/eventDispatcher';
import { extractEventPayload } from '../../internal/events';
import type { PangleMediationNativeAdEvent } from '../../internal/events';
import type {
  PangleAdEventListener,
  PangleMediationAdType,
  PangleMediationError,
} from '../../types/events';

let nextAdInstanceId = 0;

export function createAdInstanceId(
...
type AdLoadState = 'idle' | 'loading' | 'loaded';
...
function toPangleMediationError(error: unknown): PangleMediationError {
 ...
 */
export abstract class BaseFullscreenAd<TEventMap> {
  protected readonly adUnitId: string;
  private readonly adType: PangleMediationAdType;
  ...
  private state: AdLoadState = 'idle';
  ...
  protected getInternalInstanceId() {
    return this.instanceId;
  }

  private handleNativeEvent = (event: PangleMediationNativeAdEvent) => {
  ...
  private notifyLocalLoadFailed(error: PangleMediationError) {
    this.notifyLocalError('load_failed', error);
  }

  private notifyLocalShowFailed(error: PangleMediationError) {
    this.notifyLocalError('show_failed', error);
  }

  private notifyLocalError(type: string, error: PangleMediationError) {
  ...
  private ensureSubscribed() {
    if (this.dispatcherUnsubscribe == null) {
      this.dispatcherUnsubscribe = adEventDispatcher.subscribe(
        this.instanceId,
  ...
  protected performLoad(invokeNativeLoad: () => void): void {
    if (this.state !== 'idle') {
      this.notifyLocalLoadFailed(
        toPangleMediationError(
          new PangleAdGuardError(
            AD_INSTANCE_IN_USE,
            'load() requires a unique internal ad instance while a matching ad is still loaded or loading'
          )
        )
      );
      return;
  ...
  protected performShow(invokeNativeShow: () => void): void {
    if (this.state !== 'loaded') {
      this.resetToIdle();
      this.notifyLocalShowFailed(
        toPangleMediationError(
          new PangleAdGuardError(AD_NOT_READY, 'The ad is not loaded or ready')
        )
      );
      return;
    }
   ...
   */
  addAdEventListener<K extends keyof TEventMap & string>(
    type: K,
    listener: PangleAdEventListener<TEventMap, K>
  ): () => void {
    this.ensureSubscribed();
